...
shot_num = 'G43798'
file = 'profile/200910_G43798_HOPG_1505'
Energy = 1.325 #kJ

data = np.genfromtxt('{}.csv'.format(file), delimiter=',', skip_header= 1, skip_footer= 0, autostrip=True)

E_def = np.array([8.048, 8.391012])										#基準の線のエネルギー
theta_def = np.arcsin(12.3984/(2.0 * d * E_def))						#基準の線のブラッグ角
place = np.array([1.005, 1.52])											#基準の線のIP上での位置[cm]

	#  theta = np.arctan((13 + x - place)/(50 - y))

left = [[1,np.tan(theta_def[0])],
			[1,np.tan(theta_def[1])]]

right = [50 * np.tan(theta_def[0]) + place[0] - 13, 50 * np.tan(theta_def[1]) + place[1] - 13]
